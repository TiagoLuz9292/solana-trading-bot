...
const connection = new web3.Connection(
    web3.clusterApiUrl('https://serene-soft-dream.solana-mainnet.quiknode.pro/d9545d21916469751695fb7a165e97325634fdb5'), 
    'confirmed'
);
...
async function swap_from_sol_to_token(amount_sol : number, token_Address : String) {
    ...
    const amount_sol_to_buy = amount_sol * 1000000000;
    console.log("test " + token_Address);
...
async function waitForTransactionConfirmation(signature: TransactionSignature, connection: Connection): Promise<number> {
    ...
    while (true) {
        ...
        try {
            ...
            if (response && response.meta && !response.meta.err) {
                ...
                delay = 5000; // Reset delay after successful processing
            } else {
            ...
            if (error.statusCode === 429 || error.message?.includes('429')) {
                console.log(`Server responded with 429 Too Many Requests. Retrying after ${delay / 1000} seconds...`);
                delay = Math.min(delay * 2, maxDelay); // Increment delay, capping at maxDelay
            } else {
                console.error("An error occurred:", error);
                return 0; // Return 0 if there is an error other than 429
            }
        }

        await new Promise(resolve => setTimeout(resolve, delay));
    }
    console.log(`DEBUG: Transaction confirmation loop ended. Token amount received: ${tokenAmountReceived}`);
    return tokenAmountReceived ?? 0; // Return 0 if tokenAmountReceived is null
}
...
async function pre_and_post_sell_operations(token_amount: number, token_address: String): Promise<void> {
    try {
        ...
        const sol_received = await waitForTransactionConfirmation(signature, connection);
