    ...
    while (true) {
        try {
            ...
            if (response && response.meta && !response.meta.err) {
                ...
                console.log(`Transaction ${signature} is not confirmed yet, checking again in ${delay / 5000} seconds...`);
                delay = 5000;
            }
        } catch (error: any) {
            if (error.statusCode === 429 || error.message?.includes('429')) {
                ...
                break; // Exit on other types of errors
            }
        }
...
async function pre_and_post_buy_operations(amount_usd: number, amount_sol: number, token_address: String) {
    try {
        ...
        console.log("INFO: Performing swap from " + amount_sol + " SOL ("+amount_usd+" USD) to " + token_address);
        ...
        const csvFilePath = "/home/tluz/project/ON-CHAIN-SOLANA-TRADING-BOT/data/buy_tracker_testing_confirmed.csv";
